...
export function EstimatesView({
  ...
  const handleDelete = async () => {
    ...
    try {
      const {
        error
      } = await supabase.from("estimates").delete().eq("id", deleteEstimate);
      if (error) throw error;
  ...
  const handleEdit = async (id: string) => {
    try {
      const {
        data,
        error
      } = await supabase.from("estimates").select("*").eq("id", id).single();
      if (error) throw error;
  ...
  if (editingEstimate) {
    return <div className="space-y-6">
        ...
        <EstimateForm initialData={editingEstimate} onSubmit={handleUpdate} />
      </div>;
  }
  return <div className="space-y-6 px-[24px] py-[24px]">
      ...
      <EstimatesList companyId={companyId} onEdit={handleEdit} onDelete={setDeleteEstimate} />
